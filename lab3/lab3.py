...
def encrypt():
    ...
    salt = b'\xb5d\x1c\x0c1\xa1\x9da\xee\xc2N/f\xf7 C'
    password = input("Enter your secret key: ")
    rand = str(random.randint(1, 999999))
    password = password + rand
    print("password: ",password)
    hash_pass = hashlib.md5(password.encode())
    hash_pass = str(hash_pass.hexdigest())
    key = PBKDF2(hash_pass, salt, dkLen=16)
    inp_f = open('output.csv', 'rb')
    out_f = open('encrypt.txt', 'wb')
    cipher_encrypt = AES.new(key, AES.MODE_CFB)

    out_f.write(cipher_encrypt.iv)
    buffer = inp_f.read()
    while len(buffer) > 0:
        ciphered_bytes = cipher_encrypt.encrypt(buffer)
        out_f.write(ciphered_bytes)
        buffer = inp_f.read()
    inp_f.close()
    out_f.close()
    return salt
...
def decrypt():
    ...
    password = input("Enter your secret key: ")
    get_hash = hashlib.md5(password.encode())
    get_hash = str(get_hash.hexdigest())
    key = PBKDF2(get_hash, salt, dkLen=16)
    inp_f = open('encrypt.txt', 'rb')
    out_f = open('output.csv', 'wb')
    cipher_encrypt = AES.new(key, AES.MODE_CFB, iv=inp_f.read(16))
    buffer = inp_f.read()
    while len(buffer) > 0:
